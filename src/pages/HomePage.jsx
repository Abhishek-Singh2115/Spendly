...
export const HomePage = ({ ctx }) => {
  ...
  // Top categories for selected month (NO accountId filter)
  const catMap = {};
  monthTxns.filter(t => t.type === 'expense').forEach(t => {
    const c = t.category || 'other';
    catMap[c] = (catMap[c] || 0) + (parseFloat(t.amount) || 0);
  });
  const categoryBreakdown = Object.entries(catMap).sort((a, b) => b[1] - a[1]).slice(0, 3);
      ...
      <div style={{ padding: '0 18px 16px' }}>
        ...
        <div className="scroll-x">
          <div className="quick-action" onClick={() => accounts.length ? navigate('addExpense', accounts[0]) : navigate('addAccount')}>
            <div className="qa-icon">💸</div><span>Add Expense</span>
          </div>
